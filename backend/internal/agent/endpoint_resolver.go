...
//
//	keystone → http://keystone.opsl2.svc.cluster.local
//	nova     → http://nova.opsl2.svc.cluster.local
//	cinder   → http://cinder.opsl2.svc.cluster.local
//	neutron  → http://neutron.opsl2.svc.cluster.local
//	glance   → http://glance.opsl2.svc.cluster.local
//	emla     → http://emla.opsl2.svc.cluster.local   (monitoring/observability)
//	heat     → http://heat.opsl2.svc.cluster.local
//	octavia  → http://octavia.opsl2.svc.cluster.local
...
// Priority:
//  1. HostIP + BaseDomain → construct "http://<service>.<base_domain>"
...
func ResolveEasyStackEndpoints(p model.CloudPlatform) EasyStackServiceEndpoints {
	...
	if p.HostIP != "" && p.BaseDomain != "" {
		...
		ep := EasyStackServiceEndpoints{
			Keystone:   fmt.Sprintf("http://keystone.%s", bd),
			Nova:       fmt.Sprintf("http://nova.%s", bd),
			Cinder:     fmt.Sprintf("http://cinder.%s", bd),
			Neutron:    fmt.Sprintf("http://neutron.%s", bd),
			Glance:     fmt.Sprintf("http://glance.%s", bd),
			Emla:       fmt.Sprintf("http://emla.%s", bd),
			Heat:       fmt.Sprintf("http://heat.%s", bd),
			Octavia:    fmt.Sprintf("http://octavia.%s", bd),
			Ceilometer: fmt.Sprintf("http://ceilometer.%s", bd),
		}

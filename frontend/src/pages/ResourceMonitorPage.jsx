...
export default function ResourceMonitorPage() {
  ...
  // Alert filter & pagination state
  const [alertFilter, setAlertFilter] = useState('firing'); // 'firing' | 'resolved'
  ...
  const filteredAlerts = useMemo(() => {
    ...
    return data.alerts.filter(a => a.state === alertFilter);
  }, [data?.alerts, alertFilter]);
  ...
  }, [filteredAlerts, alertPage, ALERTS_PER_PAGE]);
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className={`flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-full border transition-all cursor-pointer ${
                      alertFilter === 'firing'
                        ? 'bg-red-100 text-red-700 border-red-300 shadow-sm font-semibold'
                        : 'bg-red-50 text-red-500 border-red-100 hover:bg-red-100 hover:border-red-200'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-full border transition-all cursor-pointer ${
                      alertFilter === 'resolved'
                        ? 'bg-emerald-100 text-emerald-700 border-emerald-300 shadow-sm font-semibold'
                        : 'bg-emerald-50 text-emerald-500 border-emerald-100 hover:bg-emerald-100 hover:border-emerald-200'
                    }`}
                  >
                ...
                <div>
                    ...
                    <table className="w-full text-sm table-fixed">
                      <thead>
                        <tr className="bg-gray-50 border-b border-gray-100">
                          <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[60px]">状态</th>
                          <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[140px]">已接入云平台</th>
                          <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[100px]">告警优先级</th>
                          <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">告警内容</th>
                          <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[140px]">告警对象</th>
                          <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[110px]">时间</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        {paginatedAlerts.map((alert, i) => (
                          ...
                          >
                            ...
                            {/* Cloud platform */}
                            <td className="px-4 py-3" title={alert.platform || '-'}>
                              <div className="flex items-center gap-1.5 min-w-0">
                                <Cloud className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />
                                <span className="text-sm text-gray-700 font-medium truncate">{alert.platform || '-'}</span>
                              </div>
                            </td>
                            {/* Severity */}
                            <td className="px-4 py-3" title={alert.severity === 'critical' ? '严重' : alert.severity === 'warning' ? '警告' : alert.severity === 'info' ? '信息' : '-'}>
                              {alert.severity ? (
                            ...
                            {/* Alert target */}
                            <td className="px-4 py-3" title={alert.target || '-'}>
                              <span className="text-xs text-gray-500 font-mono bg-gray-50 px-1.5 py-0.5 rounded truncate block max-w-full">
                                {alert.target || '-'}
                              </span>
                            </td>
                            {/* Timestamp */}
                            <td className="px-4 py-3 whitespace-nowrap">
                              {alert.timestamp ? (
                                <span className="text-xs text-gray-400" title={new Date(alert.timestamp).toLocaleString('zh-CN')}>
                  ...
                  {totalAlertPages > 1 && (
                    <div className="flex items-center justify-between mt-3 px-1">
                      <span className="text-xs text-gray-400">
                        共 {filteredAlerts.length} 条{alertFilter === 'firing' ? '告警中' : '已恢复'}告警
                      </span>
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => setAlertPage(p => Math.max(1, p - 1))}
                          disabled={alertPage <= 1}
                          className="p-1.5 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 hover:border-gray-300 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                        >
                          <ChevronLeft className="w-3.5 h-3.5" />
                        </button>
                        {Array.from({ length: totalAlertPages }, (_, i) => i + 1)
                          .filter(p => p === 1 || p === totalAlertPages || Math.abs(p - alertPage) <= 1)
                          ...
                          .map((p, idx) =>
                            p === '...' ? (
                              ...
                              <button
                                ...
                                className={`min-w-[28px] h-7 text-xs rounded-lg border transition-all ${
                                  alertPage === p
                                    ? 'border-[#513CC8] bg-[#EEE9FB] text-[#513CC8] font-semibold'
